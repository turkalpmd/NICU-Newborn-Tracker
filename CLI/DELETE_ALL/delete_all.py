...
logger.success("boto3 version: " + boto3.__version__)
...
def run():
    ...
    for x in response['Regions']:
        ...
        if region_name in common_regions:
...
def delete_db_cluster_snapshots(region_name):
    ...
    logger.success("All DB cluster snapshots deleted successfully!")
...
def delete_ecs_tasks(region_name):
    ...
    # Deregister each task and wait for completion
    ...
    for task_arn in task_arns:
        task_arn_split = task_arn.split(":")
        task_id = task_arn_split[-1]
        ecs_client.stop_task(
            task=task_id
        )
        logger.warning('Stopping ECS task: {}'.format(task_id))
        ecs_client.delete_task_definition(
            taskDefinition=task_arn
        )
        logger.warning('Deleting ECS task: {}'.format(task_arn))
...
def delete_all_redshift_clusters(region_name):
    ...
    for cluster in response['Clusters']:
        ...
        logger.warning(f"Deleting Redshift cluster:{cluster_identifier}")
        try:
...
def delete_peering_connection(region_name):
    ...
    for peering_connection in peering_connections['VpcPeeringConnections']:
        ...
        try:
            ...
            logger.critical('Failed while deleting Peering Connection:', peering_connection['VpcPeeringConnectionId'])
...
def delete_load_balancer_listener(region_name):
    ...
    for lb in res['LoadBalancers']:
        ...
        for listener in listeners_response['Listeners']:
            ...
            waiter = client.get_waiter('listener_not_exists')
            try:
                waiter.wait(ListenerArn=listener['ListenerArn'])
                logger.error('Listener deleted:', listener['ListenerArn'])
            except WaiterError as e:
                logger.critical('Error waiting for listener to be deleted:', e)

    for i in res["LoadBalancers"]:
        logger.warning("LoadBalancer Listener found!!!")

        if i.get("LoadBalancerArn") is not None:
            logger.warning("ARN: " + i.get("LoadBalancerArn"))
            res1 = client.describe_listeners(
                LoadBalancerArn=i.get("LoadBalancerArn")
            )
            for ii in res1["Listeners"]:
                client.delete_listener(
                    ListenerArn=ii.get('ListenerArn')
                )
                waiter = client.get_waiter('listener_not_exists')
                waiter.wait(ListenerArns=[ii.get('ListenerArn')])
...
def delete_network_interface(region_name):
    ...
    for network_interface in network_interfaces['NetworkInterfaces']:
        ...
        try:
            ...
            ni_waiter = ec2.get_waiter('network_interface_deleted')
            ni_waiter.wait(NetworkInterfaceIds=[ni_id])
            logger.success(f"Network interface {ni_id} deleted successfully")
        except Exception as e:
            logger.error(f"Error waiting for network interface {ni_id} deletion: {e}")
...
def delete_sgr(region_name):
    ...
    for security_group in response['SecurityGroups']:
        ...
        if len(security_group['IpPermissions']) > 0:
            ...
            # waiter = ec2.get_waiter('security_group_rule_deleted')
            # waiter.wait(Filters=[
            #     {'Name': 'group-id', 'Values': [group_id]},
            #     {'Name': 'ip-permission.protocol', 'Values': ['tcp', 'udp', 'icmp']}
            # ])
...
def delete_db_instances(region):
    ...
    for instance in res['DBInstances']:
        ...
        logger.warning(f"Waiting for RDS instance {instance_identifier} to be deleted...")
        try:
...
def delete_rds(region_name):
    ...
    if dbi_count > 0:
        for x in res2["DBInstances"]:
            logger.warning("deleting DB Instance: " + x["DBInstanceIdentifier"] + ". Please wait... (apx 5-12 minutes)")
...
def delete_lambda_functions(region_name):
    ...
    if len(function_names) < 1:
        ...
        return
    logger.info(f"Lambda Function Found{len(function_names)}")
    ...
    for igw in res["InternetGateways"]:
        ...
        for att in igw["Attachments"]:
            ...
            try:
                ...
                raise
        logger.info(f"Deleting Internet Gateway: {igw_id}")
        try:
